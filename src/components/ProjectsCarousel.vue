  ...
  <Carousel
    ...
    <Slide v-for="(item, key) in props.data" :key="key" class="w-full h-full">
      ...
      <div class="flex items-center justify-between w-11/12 h-full gap-2 mx-auto my-3 border-t border-black lg:w-10/12 2xl:w-8/12">
        <button @click="goPrev" class="flex items-center flex-1 gap-2 group">
          ...
          <span class="font-[800] max-sm:text-[12px] text-center">{{ prevSlideTitle }}</span>
        </button>
        <button @click="goNext" class="flex items-center flex-1 gap-2 group">
          <span class="font-[800] max-sm:text-[12px] text-center">{{ nextSlideTitle }}</span>
          <span class="p-3 bg-gold group-hover:bg-olive focuse:bg-olive">
