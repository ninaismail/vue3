...
<template>
  <div @mouseover="handleMouseOver" @mouseleave="handleMouseLeave" class="fixed z-[10] inset-0 w-full h-[64px] shadow-md">
    ...
    <nav class="items-center justify-center hidden w-8/12 h-full gap-2.5 lg:flex">
      ...
      class="uppercase cursor-pointer min-w-[193px] font-[700] text-accent1 hover:font-[700] text-center">
        {{ item.name }}
      </RouterLink>
    </nav>
    <div class="flex gap-2.5 items-center lg:pe-[8.67%] pe-0">
    ...
    <nav v-if="isOpen" class="flex flex-col w-full py-5 mt-5 space-y-4 lg:hidden">
      <RouterLink v-for="(item, key) in navlinks" :key="key" :id="item.name + (isClicked === item.id ? '-active' : '')" :aria-label="'go to ' + item.name" :to="item.to"
      class="cursor-pointer font-[700] text-accent1 hover:font-[700] hover:drop-shadow-md">
        {{ item.name }}
      </RouterLink>
    </nav>
  </header>  
